    ...
    public partial class frmInvoice : Form
    {
        ...
        private void Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                String Words = Spell.SpellAmount.InWrods(Convert.ToDecimal(  txtTotal.Text   ));
                Words = "NRS" + Words.Remove(0, 4);              

                // Generate Nepali Date
                String InVoiceD=DipeshNepaliDateConversion.DipeshNepaliDateConverter.convertToBS(dtpInvoiceDate.Value );
                MessageBox.Show(InVoiceD);


                con = new OleDbConnection(cs);
                con.Open();

                string cb = "insert Into Sales(InvoiceNo,InvoiceDate,CustomerID,SubTotal,VATPercentage,VATAmount,GrandTotal,TotalPayment,PaymentDue,Remarks,PaymentInWords, InvoiceDateNepali) VALUES ('" + txtInvoiceNo.Text + "',#" + dtpInvoiceDate.Text + "#,'" + txtCustomerID.Text + "'," + txtSubTotal.Text + "," + txtTaxPer.Text + "," + txtTaxAmt.Text + "," + txtTotal.Text + "," + txtTotalPayment.Text + "," + txtPaymentDue.Text + ",'" + txtRemarks.Text + "', ' " + Words + "', '" + InVoiceD + "')";
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            con = new OleDbConnection(cs);
            con.Open();
            String cb = "update Sales set GrandTotal= " + txtTotal.Text + ",TotalPayment= " + txtTotalPayment.Text + ",PaymentDue= " + txtPaymentDue.Text + ",Remarks='" + txtRemarks.Text + "' where Invoiceno= '" + txtInvoiceNo.Text + "'";
            cmd = new OleDbCommand(cb);
            cmd.Connection = con;
            cmd.ExecuteReader();
            con.Close();
            ...
            { 
                ...
                con = new OleDbConnection(cs);
                string cd = "update ProductSold set Quantity=" + ListView1.Items[i].SubItems[4].Text + ",Price= " + ListView1.Items[i].SubItems[3].Text + ",TotalAmount= " + ListView1.Items[i].SubItems[5].Text + " where InvoiceNo='" + txtInvoiceNo.Text + "' and ConfigID= " + ListView1.Items[i].SubItems[1].Text + "";
                cmd = new OleDbCommand(cd);
                cmd.Connection = con;
                con.Open();
                cmd.ExecuteNonQuery();
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                String newVal = Microsoft.VisualBasic.Interaction.InputBox("Please Enter Amount", "Remaining Amount", "");
                if (newVal != "")
                {
                    int oldVal = Convert.ToInt32(txtTotalPayment.Text);
                    txtTotalPayment.Text = Convert.ToString(oldVal + Convert.ToInt32(newVal));
                }
            }
            catch( Exception ex)
            {
        ...
        private void chkClearPayment_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                if (chkClearPayment.Checked == true)
                {
                    oldValun = Convert.ToInt32(txtTotalPayment.Text);
                    txtTotalPayment.Text = txtTotal.Text;
                }
                if (chkClearPayment.Checked == false)
                {
                    txtTotalPayment.Text = Convert.ToString(oldValun);
                }
            }
            catch (Exception ex)
            {
